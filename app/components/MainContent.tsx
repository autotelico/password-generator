...
export default function MainContent(): React.ReactNode {
  ...
  const copyPasswordToClipboard = () => {
    const pwInput = document.querySelector<HTMLInputElement>('#password-input')!.value
    ...
    <main className='bg-slate-200 py-10'>
      <div className="w-[300px] sm:w-[500px] h-[500px] sm:h-auto relative left-[50%] translate-x-[-50%]">
        ...
        <div className='flex flex-col sm:flex-row justify-center items-center gap-3'>
            <input type="text" value={password} id='password-input' className='rounded-md p-1'/>
            {pwWasCopied ? <TbCopyCheck size={30} onClick={copyPasswordToClipboard}/> : <TbCopy size={30} onClick={copyPasswordToClipboard}/>}
        </div>
        <button
        ...
        <div className='flex justify-evenly gap-5'>
            <button onClick={() => updatePwLength('+')} className='size-[50px] bg-[#242424] text-[#eee] px-4 py-2 rounded-md font-semibold'>+</button>
            <div>
